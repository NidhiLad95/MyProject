    ...
    public class VectorStoreSeedService : IVectorStoreSeedService
    {
        ...
        public async Task SeedAsync(SQLiteVectorStore store, string connectionString, string? schemaName,string? TablesSelected, string ViewsSelected, CancellationToken ct)
        {
            ...
            foreach (var kv in dict)
            {
                ...
                var entry = new SchemaEntry(tableName, schemaJson, embedding);
        ...
        CancellationToken ct = default)
        {
            ...
            foreach (var path in filePaths)
            {
                ...
                string text = GenxAi_Solutions.Utils.TextExtractor.ExtractTextFromPdf(path); // :contentReference[oaicite:2]{index=2}

                // 2) Split into sections/chunks (your util)
                var sections = GenxAi_Solutions.Utils.TextExtractor.SplitIntoSections(text, sectionMaxWords); // :contentReference[oaicite:3]{index=3}

                int sectionIdx = 0;
                foreach (var section in sections)
                {
                    var chunks = GenxAi_Solutions.Utils.TextExtractor.ChunkText(section, chunkMaxWords); // :contentReference[oaicite:4]{index=4}
        ...
        CancellationToken ct = default)
        {
            ...
            var sections = GenxAi_Solutions.Utils.TextExtractor.SplitIntoSections(text, sectionMaxWords); // :contentReference[oaicite:5]{index=5}

            int sectionIdx = 0;
            foreach (var section in sections)
            {
                var chunks = GenxAi_Solutions.Utils.TextExtractor.ChunkText(section, chunkMaxWords); // :contentReference[oaicite:6]{index=6}
