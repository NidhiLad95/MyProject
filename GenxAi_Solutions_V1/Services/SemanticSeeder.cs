...
namespace GenxAi_Solutions_V1.Services
{
    public class SemanticSeeder : ISemanticSeeder
    {
        ...
        public async Task RunSeedAsyncNew(int companyId, CancellationToken ct)
        {
            ...
            // 3) create/open store
            var store = _storeFactory.Create_New(sqliteDbFile);

            try
            {
                // 4) seed schema + columns
                await _seedService.SeedAsyncNew(store, cfg.ConnectionString!, cfg.DatabaseName, cfg.TablesSelected, cfg.ViewsSelected, ct);

                // 5) seed prompts
                await _seedService.SeedPromptsAsyncNew(store, cfg.PromptConfiguration, ct);
        ...
        public async Task RunSeedPDFAsync_New(int companyId, CancellationToken ct)
        {
            _log.LogInformation("RunSeedPDFAsync start company={CompanyId}", companyId);
            _audit.LogGeneralAudit("Seed.PDF.Start", "system", "-", $"company={companyId}");
            ...
            var filePaths = files
                ...
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .ToArray();
            var store = _storeFactory.PdfCreate_New(sqliteDbPath);
            ...
            try
            {
                ...
                if (filePaths.Length > 0)
                {
                    foreach (var fl in filePaths)
                    {
                        await _seedService.IngestPdfAsyncNew(store,fl?.ToString() ?? "", sqliteDbPath, "book_");
                    }
                ...
                if (promptDict.Count > 0)
                {
                    // requires an overload like this in your seed service
                    //await _seedService.SeedPdfPromptsAsync(sqliteDbPath, promptDict, ct);
                    await _seedService.SeedPdfPromptsAsyncNew(store, promptTexts.FirstOrDefault().ToString(), ct);
                }
