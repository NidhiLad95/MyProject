...
ORDER BY table_schema, table_name, ordinal_position;";
        ...
        public static DataTable ExecuteQueryForChat1(string dbType, string connectionString, string databaseName, string sql)
        {
            var dt = new DataTable();
            switch (dbType)
            {
                case "SQL Server":
                    using (var conn = new SqlConnection(connectionString))
                    {
                        ...
                        Console.WriteLine("Before change DB: " + conn.Database);
                        conn.ChangeDatabase(databaseName);
                        Console.WriteLine("After change DB: " + conn.Database);
        ...
        public static DataTable ExecuteQueryForChat(string dbType, string connectionString, string databaseName, string sql)
        {
            ...
            try
            {
                ...
                // Console.WriteLine($"Executing query on {dbType}, DB: {databaseName}");
                ...
                switch (dbType)
                {
                    case "SQL Server":
                        using (var conn = new SqlConnection(connectionString))
                        {
                            ...
                            conn.StatisticsEnabled = false; // Disable statistics collection

                            conn.Open();

                            // Remove or comment out Console.WriteLine for production
                            // Console.WriteLine("Before change DB: " + conn.Database);
                            ...
                            using (var cmd = new SqlCommand("SET NOCOUNT ON; " + sql, conn))
                            {
                                ...
                                cmd.CommandTimeout = 30; // Reduced from default 30 seconds
                                ...
                                using (var adapter = new SqlDataAdapter(cmd))
                                {
                                    try
                                    {
                                        ...
                                        adapter.Fill(0, 200, dt); // Get first 200 rows only

                                        // Log performance if query took > 500ms
                                        stopwatch.Stop();
                                        if (stopwatch.ElapsedMilliseconds > 500)
                                        {
                                            Console.WriteLine($"SQL Server query took {stopwatch.ElapsedMilliseconds}ms, rows: {dt.Rows.Count}");
                                        }
                                    }
                                    catch (Exception ex)
                                    {
                                        stopwatch.Stop();
                                        Console.WriteLine($"SQL Error after {stopwatch.ElapsedMilliseconds}ms: " + ex.Message);
                                        throw;
                                    }
                                }
                            }
                        ...
                        using (var conn = new MySqlConnection(connectionString))
                        {
                            ...
                            using (var cmd = new MySqlCommand(sql, conn))
                            {
                                ...
                                using (var adapter = new MySqlDataAdapter(cmd))
                                {
                                    ...
                                    if (stopwatch.ElapsedMilliseconds > 500)
                                    {
                                        Console.WriteLine($"MySQL query took {stopwatch.ElapsedMilliseconds}ms, rows: {dt.Rows.Count}");
                                    }
                                }
                            }
                        }
                        ...
                        using (var conn = new NpgsqlConnection(connectionString))
                        {
                            ...
                            using (var cmd = new NpgsqlCommand(sql, conn))
                            {
                                ...
                                using (var adapter = new NpgsqlDataAdapter(cmd))
                                {
                                    ...
                                    if (stopwatch.ElapsedMilliseconds > 500)
                                    {
                                        Console.WriteLine($"PostgreSQL query took {stopwatch.ElapsedMilliseconds}ms, rows: {dt.Rows.Count}");
                                    }
                                }
                            }
                        }
            ...
            {
                stopwatch.Stop();
                Console.WriteLine($"Database query failed after {stopwatch.ElapsedMilliseconds}ms: {ex.Message}");
                throw;
            }
        }
        public static async Task<List<Models.SchemaEntry>> GetSchemaDict_UpNew(string connectionString, string db, List<string> tbl, List<string> vw)
        {
            //var schema = new Dictionary<string, List<Dictionary<string, string>>>();
            var map = new List<Models.SchemaEntry>();
...
ORDER BY s.name, t.name, c.column_id;";
            ...
            void Flush()
            {
                ...
                {
                    map.Add(new Models.SchemaEntry
                    {
                        TableName = currentTable,
                        SchemaText = sb.ToString()
                    });
                }
            }
            ...
            while (await reader.ReadAsync())
            {
                ...
                var isPk = reader.GetInt32(6) == 1 ? "PK" : "";

                sb.AppendLine($" - {columnName} {typeName}({maxLen}) {nullable} {isPk}".Trim());
            }
